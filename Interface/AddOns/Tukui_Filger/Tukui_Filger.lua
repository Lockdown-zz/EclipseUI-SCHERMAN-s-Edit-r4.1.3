		...
		if (not bar) then
			...
			if (self.Mode == "ICON") then
				...
				if (bar.count) then
					...
					bar.count:SetFont(C["media"].dmgfont, 8, "OUTLINEMONOCHROME")
					bar.count:SetTextColor(0, 1, 0)
					bar.count:Point("BOTTOMRIGHT", 1, 2)
				...
				if (bar.time)then
					...
					bar.time:SetFont(C["media"].uffont, 18, "OUTLINEMONOCHROME")
				...
				if (bar.count) then
					...
					bar.count:SetFont(C["media"].uffont, 13, "OUTLINEMONOCHROME")
				...
				if (bar.spellname)then
					...
					bar.spellname:SetFont(C["media"].uffont, 18, "OUTLINEMONOCHROME")
